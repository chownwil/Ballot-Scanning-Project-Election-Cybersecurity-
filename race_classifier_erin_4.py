...
def rect_detect(filepath):	
	bubbles = ballotToCrop["Erin Maureen Taylor"]
...
def main():
		...
		for in_folder in range(100):
			...
			for im_num in range(100):		
				...
				if os.path.exists(image_path):
					...
					jpg_number += 1
					if pagetype_name != "Erin Maureen Taylor":
						continue
					else:
						count += 1
						bubbles = rect_detect(image_path)
	...
	outputdf = pd.read_csv('output_erin.csv')
	for i in range(len(all_classifications)):
		outputdf['CNN'][i] = all_classifications[i]
	outputdf.to_csv('output_erin.csv', index=False)
